...
namespace HardwareMonitor.Hardware
{
    public class Visitor : IVisitor
    {
        ...
        public void VisitHardware(IHardware hardware)
        {
            ...
            {
                Metrics.CpuTemp = tctlSensor.Value.Value;
            }
            else
            {
        ...
        private void TryExtractPrimaryCpuSensors(IHardware hardware)
        {
            if (!Metrics.CpuTemp.HasValue || Metrics.CpuTemp <= 0)
            {
                ...
                if (tctlSensor?.Value.HasValue == true)
                    Metrics.CpuTemp = tctlSensor.Value.Value;
            }
        ...
        private void ExtractCpuMetrics(IHardware cpu)
        {
            ...
            if (tctlSensor != null && tctlSensor.Value.HasValue && tctlSensor.Value.Value > 0)
            {
                Metrics.CpuTemp = tctlSensor.Value.Value;
                return;
            }
        ...
        private void ExtractGpuMetrics(IHardware gpu)
        {
            ...
            if (gpuLoad?.Value.HasValue == true)
                Metrics.GpuLoad = gpuLoad.Value.Value;
        ...
        private void TryExtractCpuTempFromGenericSensors(IHardware hardware)
        {
            ...
            var sensor = temperatureSensors
                ...
                {
                    Sensor = s,
                    Score = GetCpuSensorScore(hardware, s)
                })
                .Where(x => x.Score > 0)
                .OrderByDescending(x => x.Score)
                .ThenByDescending(x => x.Sensor.Value.Value)
                .FirstOrDefault();

            if (sensor != null && sensor.Sensor.Value.HasValue)
            {
                Metrics.CpuTemp = sensor.Sensor.Value.Value;
                return;
            }
        }
        ...
        private static int GetCpuSensorScore(IHardware hardware, ISensor sensor)
        {
            ...
            if (ContainsAny(sensor.Name, "tctl", "tdie", "cpu package", "package", "cpu", "ccd"))
                score += 12;
